...
# Collects Terraform evidence files, hashes them, bundles them,
# and uploads to an S3 Object Lock vault.
#
# Usage:
...
# Create temp directory, clean it up on exit no matter what
WORK=$(mktemp -d)
trap 'rm -rf "$WORK"' EXIT

# Use whichever hash tool is available
...
# Build manifest — hash and size of every file
{
  echo "["
  FIRST=1
...
# Print receipt — this is what CI pipelines capture
